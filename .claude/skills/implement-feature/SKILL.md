...
2. **One task at a time.** Work strictly in tasks.md order (parallelize only `[P]` tasks). For each task: implement → run its done-check → tick the checkbox in tasks.md → conventional checkpoint commit. Use a **lowercase-leading subject and put the task id in the body** — `feat(slug): add the store factory` with body `T3: …`, never `feat(slug): T3 …` (commitlint's `subject-case` rejects an uppercase-leading subject — see `docs/conventions/git.md`). The ticked file is the durable progress record — after any context compaction, re-read tasks.md to recover state instead of guessing.
