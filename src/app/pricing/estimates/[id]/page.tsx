...
/** Edit an estimate with the wizard (spec 045). Leadership only. */
export default async function EditEstimatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [existing, items, types, projects, showMembers] = await Promise.all([
    getEstimate(id),
    listPricingItems(),
    listProjectTypes(),
    listProjects(),
  ...
  if (!existing) redirect("/pricing/estimates");
  return (
    <AppShell
      title="Pricing"
    ...
    >
      <EstimateWizard
        items={items}
        projectTypes={types.filter((t) => t.active)}
        existing={existing}
      />
    </AppShell>
  );
