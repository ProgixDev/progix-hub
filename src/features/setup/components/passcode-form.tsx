...
export function SetupPasscodeForm({ token }: { token: string }) {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="border-line-1 bg-card mx-auto mt-16 w-full max-w-sm rounded-xl border p-6"
    >
      ...
      <input
        ...
        className="bg-bg-inset border-line-1 focus:border-line-blue text-text mt-4 w-full rounded-md border px-3 py-2 text-center font-mono text-[16px] tracking-widest uppercase outline-none focus:ring-2 focus:ring-[var(--blue-ring)]"
      />
      {error && <p className="text-red-text mt-2 text-[12px]">{t("gateError")}</p>}
      <button
        type="submit"
        disabled={pending}
        className="bg-blue text-primary-foreground hover:bg-blue-hover mt-4 h-10 w-full rounded-md text-[14px] font-medium transition-colors disabled:opacity-60"
      >
