...
- [x] T3 `members/commits.ts`: pure `parseCommits(raw)` (newest-first, fields: sha, message, repo, url, date) + `fetchOrgCommits(login)` (Search API, `GITHUB_TOKEN`, org + author + `author-date:>=<jan1 of current year>`, capped 30, fail-soft → `[]`) · done: `commits.test.ts` green incl. empty/error/no-token (AC-5, AC-6)
...
- [x] T8 Edit `app/auth/callback/route.ts`: on the link return, stamp `github_login` into user metadata from the linked GitHub identity; map `identity_already_exists` → redirect with a friendly `?error=github_linked`; surface on sign-in/profile · done: callback unit/helper test maps conflict (AC-1, AC-7)
- [x] T9 [P] Surface a "GitHub activity" / profile entry: add `/profile` to the user menu (and sidebar if members link present) · done: signed-in user can reach `/profile` from chrome
