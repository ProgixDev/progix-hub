...
- [x] T3 `data.ts`: `getMySession()` + `listWorkStatus()` (calls `work_status_all`), server-only · done: typechecks, returns typed rows
- [x] T4 `actions.ts`: `startWorkAction/pauseWorkAction/resumeWorkAction/finishWorkAction` — `requireMember`, call RPC, `revalidatePath`, result objects · done: `actions.test.ts` covers not-authorized + a happy transition (AC-1/AC-5)
- [x] T5 `components/clock-widget.tsx` (`"use client"`): self-loads state, shows Start / running timer + Pause + Finish / Resume + Finish, ticking display · done: renders all states from a seeded state; copy in en/fr
- [x] T6 Insert `<ClockWidget/>` into the app-shell top bar once (every page) · done: widget shows on /, /members, /profile, project pages
- [x] T7 Members directory: show each member's status (working/on break/off) + hours today, merged from `listWorkStatus()` · done: directory rows show status (AC-4); `/members` page fetches it
