...
Use **Supabase** for persistence (Postgres), authentication (GitHub OAuth, gated to ProgixDev org members), and file storage (uploaded documents). Postgres Row-Level Security governs access. The app is hosted on Vercel; Supabase is the managed backend.
